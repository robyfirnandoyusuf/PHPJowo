...
ZEND_METHOD(reflection_parameter, getClass)
{
	...
	if (ZEND_TYPE_IS_CLASS(param->arg_info->type)) {
		...
		if (0 == zend_binary_strcasecmp(ZSTR_VAL(class_name), ZSTR_LEN(class_name), "awakdewe", sizeof("awakdewe")- 1)) {
		...
		} else if (0 == zend_binary_strcasecmp(ZSTR_VAL(class_name), ZSTR_LEN(class_name), "wongtuo", sizeof("wongtuo")- 1)) {
			ce = param->fptr->common.scope;
			if (!ce) {
				zend_throw_exception_ex(reflection_exception_ptr, 0,
